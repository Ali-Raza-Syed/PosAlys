...
vid_obj = VideoReader('test2.mp4');

 for i = 1 : 60
    readFrame(vid_obj); 
 end

num_of_prev_frames_to_consider = 3;
prev_locations = zeros( 3, 21, num_of_prev_frames_to_consider );
frame_num = 1;
...
%         end
    ...
    %mean wala kaam
%     if frame_num <= num_of_prev_frames_to_consider
%         prev_locations = circshift( prev_locations, -1, 3 );
%         prev_locations( :, :, num_of_prev_frames_to_consider ) = pred_p; 
%     else
%         temp = cat( 3, prev_locations, pred_p );
%         pred_p = mean( temp, 3 );
%         prev_locations = circshift( prev_locations, -1, 3 );
%         prev_locations( :, :, num_of_prev_frames_to_consider ) = pred_p;
%     end
